...
import '../../../wallet/domain/entities/chain_id.dart';
...
class PhraseRepositoryImpl extends PhraseRepository {
  ...
  @override
  Future<Wallet> deriveAllAddresses(String mnemonics) async {
  ...
  @override
  Future<void> saveData(Wallet data, String password) async {
    final salt = IV.fromSecureRandom(16);
    final secretKey = Key.fromUtf8(password).stretch(16, salt: salt.bytes);
    final jsonData = json.encode(WalletModel.fromEntity(data).toJson());
    final iv = IV.fromSecureRandom(16);
    final encrypter = Encrypter(AES(secretKey, mode: AESMode.cbc));
  ...
  @override
  Future<Wallet?> retrieveData(String password) async {
    try {
      final data = await _storage.read(key: 'data');
      ...
      var walletModel = WalletModel.fromJson(jsonData);
      // Migration: populate addresses if missing (wallet created before multi-chain support)
      if (walletModel.addresses.isEmpty) {
        final mnemonics = await _retrieveMnemonics(password);
        if (mnemonics != null) {
          final derived = await deriveAllAddresses(mnemonics);
          walletModel = walletModel.copyWith(addresses: derived.addresses);
          await saveData(walletModel.toEntity(), password);
        }
      }
